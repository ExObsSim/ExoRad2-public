...
class InstrumentDiffuseLightTest(unittest.TestCase):
    ...
    def test_transmission(self):
        ...
        wl_grid = np.logspace(np.log10(options['channel']['Phot']['detector']['wl_min']['value'].value),
                              np.log10(options['channel']['Phot']['detector']['cut_off']['value'].value), 6000) * u.um
        ...
        phot = phot.chain()
        #
        wl_grid = np.logspace(np.log10(options['channel']['Spec']['detector']['wl_min']['value'].value),
                              np.log10(options['channel']['Spec']['detector']['cut_off']['value'].value), 6000) * u.um
        telescope = OpticalPath(wl=wl_grid, description=options)
        tel = telescope.chain()
    ...
    def test_values(self):
        ...
        optics = options['channel']['Spec']['optics']['opticalElement']['optics']
        detector = options['channel']['Spec']['optics']['opticalElement']['detector']
        qe = 0.7
        ...
        detector_radiance = planck(wl=wl_grid, T=detector['temperature']['value']).to(
            u.W / (u.m ** 2 * u.micron * u.sr))
        detector_radiance *= D ** 2 * np.pi * u.sr * qe * (wl_grid / const.c / const.h).to(
            1. / u.W / u.s) * u.count
        ...
        omega_pix = OmegaPix(options['channel']['Spec']['Fnum_x']['value'].value,
                             options['channel']['Spec']['Fnum_x']['value'].value)
        optics_radiance = planck(wl=wl_grid, T=optics['temperature']['value']).to(
            u.W / (u.m ** 2 * u.micron * u.sr))
        optics_radiance *= D ** 2 * (np.pi * u.sr - omega_pix) * qe * (wl_grid / const.c / const.h).to(
            1. / u.W / u.s) * u.count
        ...
        M6_radiance = planck(wl=wl_grid, T=M6['temperature']['value']).to(u.W / (u.m ** 2 * u.micron * u.sr))
        M6_radiance *= M6['emissivity']['value']
        M6_radiance *= D ** 2 * omega_pix * qe * (wl_grid / const.c / const.h).to(
            1. / u.W / u.s) * u.count
        ...
        M5_radiance = planck(wl=wl_grid, T=M5['temperature']['value']).to(u.W / (u.m ** 2 * u.micron * u.sr))
        M5_radiance *= M5['emissivity']['value'] * M6['transmission']['value'] * D1['transmission']['value']
        ...
        M5_radiance[idx] = 0.0
        M5_radiance *= D ** 2 * omega_pix * qe * (wl_grid / const.c / const.h).to(
            1. / u.W / u.s) * u.count
        from exorad.utils.exolib import rebin
        _, M5_radiance = rebin(wl_pix, wl_grid, M5_radiance)
        M5_signal_tmp = (np.convolve(M5_radiance * dwl_pic, slit_kernel, 'same')).to(u.count / u.s)
        idx = [np.logical_and(wl_pix > wlow, wl_pix <= whigh)
               for wlow, whigh in zip(ch_table['LeftBinEdge'], ch_table['RightBinEdge'])]
        M5_signal = [M5_signal_tmp[idx[k]].value.sum() for k in range(len(idx))]
        ...
        for col in spec.signal_table.keys():
            ...
            ax.plot(wl, check_table[col], alpha=0.5, ls=':', lw=3, label='{} check'.format(col))
        ax.set_yscale('log')
        ...
        wl = spectrometer.table['Wavelength']
        for col in spec.signal_table.keys():
            ax.plot(wl, spec.signal_table[col] / check_table[col], alpha=0.5, label=col)
        ax.set_yscale('log')
        ...
        for col in spec.signal_table.keys():
            ...
            np.testing.assert_almost_equal(spec.signal_table[col].value, check_table[col], decimal=40, err_msg='',
                                           verbose=True)

        for col in spec.signal_table.keys():
            # check relative equality up to 1%
            np.testing.assert_almost_equal(spec.signal_table[col].value / check_table[col],
                                           np.ones(len(check_table[col])), decimal=2, err_msg='', verbose=True)
